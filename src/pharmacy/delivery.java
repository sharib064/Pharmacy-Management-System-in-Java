...
public class delivery extends javax.swing.JFrame {
    ...
    void update(){
        try {
            int s=Integer.parseInt(tq)-Integer.parseInt(q);
            System.out.print(s);
            Class.forName("java.sql.DriverManager");
            Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/pharmacy");
            PreparedStatement st = con.prepareStatement("UPDATE `stock` SET `quantity`='"+s+"' WHERE `name`='"+name+"' AND  `manufacturer`='"+mname+"'");
            st.executeUpdate();
        }
        catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
    ...
    void delete(){
        try {
            Class.forName("java.sql.DriverManager");
            Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/pharmacy");
            PreparedStatement st = con.prepareStatement("DELETE FROM `medorder` WHERE medicine='"+name+"'");
            st.executeUpdate();
            sale();
        }
        catch (Exception e) {
    ...
    void sale(){
        try {
                ...
                Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/pharmacy");
                java.sql.Statement st=con.createStatement();
                String sql="insert into sale values('"+name+"','"+mname+"','"+q+"','"+p+"')";
                st.execute(sql);
            }
            catch (Exception e) {
                JOptionPane.showMessageDialog(this, e.getMessage());
    ...
    private void initComponents() {
        ...
        rSButtonHover2.setText("Confirm");
    ...
    private void rSButtonHover2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_rSButtonHover2MouseClicked
        ...
        if(jComboBox1.getSelectedIndex()!=-1){
            ...
            java.sql.ResultSet rs= st.executeQuery(sql);
            while(rs.next()){
                if(rs.getString(1).equals(name) && rs.getString(2).equals(mname)){
                    tq=rs.getString(3);
                }
            }
        }
            ...
            JOptionPane.showMessageDialog(null, "First Select one");
        }
